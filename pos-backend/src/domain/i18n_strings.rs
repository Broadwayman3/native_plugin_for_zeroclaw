...
pub static TRANSLATIONS: Lazy<HashMap<&'static str, HashMap<&'static str, &'static str>>> =
    Lazy::new(|| {
        ...
        // ── en ──────────────────────────────────────────────────────────
        {
            ...
            m.insert("refund_initiated", "🔄 Refund Requested!\nInvoice #{invoice_id}\nProposal Index: {proposal_idx}");
            m.insert("refund_error", "⚠️ Refund Error: {error_msg}");
            m.insert("unsupported_currency", "❌ Error: Unsupported fiat currency '{currency}'");
            ...
            m.insert("wallet_hint", "📱 Scan with Phantom, Solflare or any Solana Wallet");
            m.insert("lang_confirm", "🌐 Interface language successfully changed to {flag} {lang_name}!");
            ...
            m.insert("select_lang", "🌐 *Select interface language / Оберіть мову:*");
            ...
            m.insert("refund_approved", "✅ Refund proposal created in Squads v4!\n• Invoice: #{invoice_id}");
            m.insert("invoice_already_cancelled", "⚠️ Invoice #{invoice_id} is already cancelled or has been paid.");
            m.insert("unauthorized_approve", "⛔ Unauthorized: Only the store manager can approve Squads v4 refund proposals.");
            m.insert("squads_refund_approved", "✅ Squads v4 Refund Proposal #{proposal_index} approved!");
            m.insert("unauthorized_reject", "⛔ Unauthorized: Only the store manager can reject Squads v4 refund proposals.");
            m.insert("squads_refund_rejected", "🚫 Squads v4 Refund proposal #{proposal_index} has been rejected. Invoice restored to 'paid'.");
            m.insert("refund_prompt", "♻️ Please enter the invoice ID to refund (e.g., INV-101):");
        ...
        // ── uk ──────────────────────────────────────────────────────────
        {
            ...
            m.insert("refund_initiated", "🔄 Ініційовано Рефанд!\nЧек #{invoice_id}\nІндекс пропозиції: {proposal_idx}");
            m.insert("refund_error", "⚠️ Помилка Рефанду: {error_msg}");
            m.insert("unsupported_currency", "❌ Помилка: Непідтримувана валюта '{currency}'");
            m.insert("receipt_title", "☕ ZeroClaw POS Чек #{invoice_id}");
            m.insert("receipt_tax", "ПДВ / Податок ({tax_rate_pct}%): ${tax_amount}");
            m.insert("receipt_total", "РАЗОМ: ${amount_usdc} USDC");
            m.insert("default_item", "Стандартне Замовлення");
            m.insert("wallet_hint", "📱 Скануйте через Phantom, Solflare або будь-який гаманець Solana");
            m.insert("lang_confirm", "🌐 Мову інтерфейсу успішно змінено на {flag} {lang_name}!");
            ...
            m.insert("select_lang", "🌐 *Оберіть мову інтерфейсу з 13 доступних:*");
            ...
            m.insert("refund_approved", "✅ Пропозицію повернення коштів створено у Squads v4!\n• Чек: #{invoice_id}");
            m.insert("invoice_already_cancelled", "⚠️ Чек #{invoice_id} вже скасовано або його сплачено.");
            m.insert("unauthorized_approve", "⛔ Заборонено: лише менеджер магазину може схвалити пропозицію повернення коштів Squads v4.");
            m.insert("squads_refund_approved", "✅ Пропозицію повернення коштів Squads v4 #{proposal_index} схвалено!");
            ...
            m.insert("refund_prompt", "♻️ Будь ласка, введіть ID чека для повернення коштів (наприклад, INV-101):");
        ...
        // ── pt ──────────────────────────────────────────────────────────
        {
            ...
            m.insert("unsupported_currency", "❌ Erro: Moeda não suportada '{currency}'");
            ...
            m.insert("wallet_hint", "📱 Escaneie com Phantom, Solflare ou qualquer carteira Solana");
            m.insert("lang_confirm", "🌐 Idioma da interface alterado para {flag} {lang_name}!");
            ...
            m.insert("refund_approved", "✅ Proposta de reembolso criada no Squads v4!\n• Fatura: #{invoice_id}");
            m.insert("invoice_already_cancelled", "⚠️ A fatura #{invoice_id} já foi cancelada ou paga.");
            m.insert("unauthorized_approve", "⛔ Não autorizado: somente o gerente da loja pode aprovar propostas de reembolso Squads v4.");
            m.insert("squads_refund_approved", "✅ Proposta de reembolso Squads v4 #{proposal_index} aprovada!");
            ...
            m.insert("refund_prompt", "♻️ Digite o ID da fatura para reembolso (ex.: INV-101):");
        ...
        // ── es ──────────────────────────────────────────────────────────
        {
            ...
            m.insert("unsupported_currency", "❌ Error: Moneda no soportada '{currency}'");
            m.insert("receipt_title", "☕ Recibo ZeroClaw POS #{invoice_id}");
            m.insert("receipt_tax", "Impuesto ({tax_rate_pct}%): ${tax_amount}");
            m.insert("receipt_total", "TOTAL: ${amount_usdc} USDC");
            m.insert("default_item", "Pedido Estándar");
            m.insert("wallet_hint", "📱 Escanea con Phantom, Solflare o cualquier billetera Solana");
            m.insert("lang_confirm", "🌐 ¡Idioma de interfaz cambiado a {flag} {lang_name}!");
            ...
            m.insert("refund_approved", "✅ Propuesta de reembolso creada en Squads v4!\n• Factura: #{invoice_id}");
            m.insert("invoice_already_cancelled", "⚠️ La factura #{invoice_id} ya está cancelada o ha sido pagada.");
            ...
            m.insert("squads_refund_approved", "✅ ¡Propuesta de reembolso Squads v4 #{proposal_index} aprobada!");
            ...
            m.insert("refund_prompt", "♻️ Ingrese el ID de la factura a reembolsar (ej.: INV-101):");
        ...
        // ── de ──────────────────────────────────────────────────────────
        {
            ...
            m.insert("refund_initiated", "🔄 Rückerstattung Beantragt!\nRechnung #{invoice_id}\nIndex: {proposal_idx}");
            m.insert("refund_error", "⚠️ Rückerstattungsfehler: {error_msg}");
            m.insert("unsupported_currency", "❌ Fehler: Nicht unterstützte Währung '{currency}'");
            ...
            m.insert("wallet_hint", "📱 Scannen Sie mit Phantom, Solflare oder einer beliebigen Solana-Wallet");
            m.insert("lang_confirm", "🌐 Schnittstellensprache erfolgreich geändert auf {flag} {lang_name}!");
            ...
            m.insert("refund_approved", "✅ Erstattungsantrag in Squads v4 erstellt!\n• Beleg: #{invoice_id}");
            m.insert("invoice_already_cancelled", "⚠️ Rechnung #{invoice_id} wurde bereits storniert oder ist bezahlt.");
            m.insert("unauthorized_approve", "⛔ Nicht autorisiert: Nur der Ladenmanager kann Squads-v4-Rückerstattungsvorschläge genehmigen.");
            m.insert("squads_refund_approved", "✅ Squads-v4-Rückerstattungsvorschlag #{proposal_index} genehmigt!");
            ...
            m.insert("refund_prompt", "♻️ Bitte geben Sie die Rechnungs-ID für die Rückerstattung ein (z. B. INV-101):");
        ...
        // ── fr ──────────────────────────────────────────────────────────
        {
            ...
            m.insert("refund_initiated", "🔄 Remboursement Demandé !\nFacture #{invoice_id}\nIndice : {proposal_idx}");
            m.insert("refund_error", "⚠️ Erreur de Remboursement : {error_msg}");
            m.insert("unsupported_currency", "❌ Erreur : Devise non prise en charge '{currency}'");
            ...
            m.insert("wallet_hint", "📱 Scannez avec Phantom, Solflare ou tout portefeuille Solana");
            m.insert("lang_confirm", "🌐 Langue de l'interface modifiée avec succès en {flag} {lang_name} !");
            ...
            m.insert("select_lang", "🌐 *Sélectionnez la langue de l'interface :*");
            ...
            m.insert("refund_approved", "✅ Proposition de remboursement créée dans Squads v4 !\n• Facture : #{invoice_id}");
            m.insert("invoice_already_cancelled", "⚠️ La facture #{invoice_id} est déjà annulée ou a été payée.");
            ...
            m.insert("squads_refund_approved", "✅ Proposition de remboursement Squads v4 #{proposal_index} approuvée !");
            ...
            m.insert("refund_prompt", "♻️ Veuillez saisir l'ID de la facture à rembourser (ex. : INV-101) :");
        ...
        // ── it ──────────────────────────────────────────────────────────
        {
            ...
            m.insert("refund_initiated", "🔄 Rimborso Richiesto!\nFattura #{invoice_id}\nIndice: {proposal_idx}");
            m.insert("refund_error", "⚠️ Errore di Rimborso: {error_msg}");
            m.insert("unsupported_currency", "❌ Errore: Valuta non supportata '{currency}'");
            ...
            m.insert("wallet_hint", "📱 Scansiona con Phantom, Solflare o qualsiasi portafoglio Solana");
            m.insert("lang_confirm", "🌐 Lingua dell'interfaccia modificata con successo in {flag} {lang_name}!");
            ...
            m.insert("refund_approved", "✅ Proposta di rimborso creata in Squads v4!\n• Fattura: #{invoice_id}");
            m.insert("invoice_already_cancelled", "⚠️ La fattura #{invoice_id} è già stata annullata o pagata.");
            ...
            m.insert("squads_refund_approved", "✅ Proposta di rimborso Squads v4 #{proposal_index} approvata!");
            ...
            m.insert("refund_prompt", "♻️ Inserisci l'ID della fattura da rimborsare (es.: INV-101):");
        ...
        // ── pl ──────────────────────────────────────────────────────────
        {
            ...
            m.insert("refund_initiated", "🔄 Żądanie Zwrotu!\nFaktura #{invoice_id}\nIndeks: {proposal_idx}");
            m.insert("refund_error", "⚠️ Błąd Zwrotu: {error_msg}");
            m.insert("unsupported_currency", "❌ Błąd: Nieobsługiwana waluta '{currency}'");
            ...
            m.insert("wallet_hint", "📱 Zeskanuj za pomocą Phantom, Solflare lub dowolnego portfela Solana");
            m.insert("lang_confirm", "🌐 Język interfejsu pomyślnie zmieniony na {flag} {lang_name}!");
            m.insert("welcome", "☕ *Witaj w terminalu ZeroClaw Solana POS!*\n\nWybierz akcję lub wpisz kwotę:");
            m.insert("custom_help", "✍️ *Wpisz kwotę i walutę w wiadomości:*\n\nPrzykłady:\n• `150 UAH`\n• `35.50 BRL`\n• `12.50 USD`\n• `2x Cappuccino + Croissant 240 UAH`");
            m.insert("price_needed", "✍️ Podaj łączną cenę i walutę dla '{items}'\n\nPrzykład:\n• `{items} 500 UAH`");
            ...
            m.insert("refund_approved", "✅ Wniosek o zwrot utworzony w Squads v4!\n• Paragon: #{invoice_id}");
            m.insert("invoice_already_cancelled", "⚠️ Paragon #{invoice_id} został już anulowany lub opłacony.");
            m.insert("unauthorized_approve", "⛔ Nieautoryzowano: tylko menedżer sklepu może zatwierdzać wnioski o zwrot Squads v4.");
            m.insert("squads_refund_approved", "✅ Wniosek o zwrot Squads v4 #{proposal_index} zatwierdzony!");
            ...
            m.insert("refund_prompt", "♻️ Podaj ID paragonu do zwrotu (np. INV-101):");
        ...
        // ── tr ──────────────────────────────────────────────────────────
        {
            ...
            m.insert("refund_initiated", "🔄 İade İstendi!\nFatura #{invoice_id}\nDizin: {proposal_idx}");
            m.insert("refund_error", "⚠️ İade Hatası: {error_msg}");
            m.insert("unsupported_currency", "❌ Hata: Desteklenmeyen para birimi '{currency}'");
            ...
            m.insert("wallet_hint", "📱 Phantom, Solflare veya herhangi bir Solana Cüzdanı ile tarayın");
            m.insert("lang_confirm", "🌐 Arayüz dili başarıyla {flag} {lang_name} olarak değiştirildi!");
            ...
            m.insert("refund_approved", "✅ Squads v4 iade teklifi oluşturuldu!\n• Fiş: #{invoice_id}");
            m.insert("invoice_already_cancelled", "⚠️ #{invoice_id} numaralı fatura zaten iptal edilmiş veya ödenmiştir.");
            m.insert("unauthorized_approve", "⛔ Yetkisiz: Squads v4 iade tekliflerini yalnızca mağaza yöneticisi onaylayabilir.");
            m.insert("squads_refund_approved", "✅ Squads v4 iade teklifi #{proposal_index} onaylandı!");
            m.insert("unauthorized_reject", "⛔ Yetkisiz: Squads v4 iade tekliflerini yalnızca mağaza yöneticisi reddedebilir.");
            m.insert("squads_refund_rejected", "🚫 Squads v4 iade teklifi #{proposal_index} reddedildi. Fatura 'paid' durumuna geri alındı.");
            m.insert("refund_prompt", "♻️ İade edilecek fatura kimliğini girin (ör. INV-101):");
        ...
        {
            let mut m = HashMap::new();
            m.insert("payment_success", "✅ 支払い完了!\n請求書 #{invoice_id}\n金額: {amount} {currency}\nTx: {tx_sig}");
            m.insert("payment_pending", "⏳ 支払い待ち...\n請求書 #{invoice_id}\n金額: {amount} {currency}\nリンク: {pay_url}\n📱 Phantom、Solflare、または任意のSolanaウォレットでスキャン");
            m.insert("refund_initiated", "🔄 返金要求!\n請求書 #{invoice_id}\nインデックス: {proposal_idx}");
            m.insert("refund_error", "⚠️ 返金エラー: {error_msg}");
            m.insert("unsupported_currency", "❌ エラー: 未対応の通貨 '{currency}'");
            ...
            m.insert("wallet_hint", "📱 Phantom、Solflare、または任意のSolanaウォレットでスキャン");
            m.insert("lang_confirm", "🌐 インターフェース言語が {flag} {lang_name} に変更されました！");
            m.insert("welcome", "☕ *ZeroClaw Solana POS Terminalへようこそ！*\n\n以下からアクションを選択するか金額を入力してください:");
            m.insert("custom_help", "✍️ *メッセージに金額と通貨を入力してください:*\n\n例:\n• `150 UAH`\n• `35.50 BRL`\n• `12.50 USD`\n• `2x Cappuccino + Croissant 240 UAH`");
            m.insert("price_needed", "✍️ '{items}' の合計価格と通貨を指定してください\n\n例:\n• `{items} 500 UAH`");
            ...
            m.insert("void_confirmed", "❌ レシート #{invoice_id} が取り消されました！");
            m.insert("refund_approved", "✅ Squads v4 返金提案が作成されました！\n• レシート: #{invoice_id}");
            m.insert("invoice_already_cancelled", "⚠️ 請求書 #{invoice_id} はすでにキャンセルされているか、支払い済みです。");
            m.insert("unauthorized_approve", "⛔ 未承認: Squads v4 の返金提案を承認できるのは店舗マネージャーのみです。");
            m.insert("squads_refund_approved", "✅ Squads v4 返金提案 #{proposal_index} が承認されました！");
            m.insert("unauthorized_reject", "⛔ 未承認: Squads v4 の返金提案を拒否できるのは店舗マネージャーのみです。");
            m.insert("squads_refund_rejected", "🚫 Squads v4 返金提案 #{proposal_index} が拒否されました。請求書は 'paid' に復元されました。");
            m.insert("refund_prompt", "♻️ 返金する請求書IDを入力してください (例: INV-101):");
        ...
        // ── zh ──────────────────────────────────────────────────────────
        {
            ...
            m.insert("wallet_hint", "📱 使用 Phantom、Solflare 或任何 Solana 钱包扫描");
            m.insert("lang_confirm", "🌐 界面语言已成功更改为 {flag} {lang_name}！");
            m.insert("welcome", "☕ *欢迎使用 ZeroClaw Solana POS 终端！*\n\n请在下方键盘选择操作或输入自定义金额：");
            m.insert("custom_help", "✍️ *请在消息中输入金额和货币：*\n\n示例：\n• `150 UAH`\n• `35.50 BRL`\n• `12.50 USD`\n• `2x Cappuccino + Croissant 240 UAH`");
            m.insert("price_needed", "✍️ 请指定 '{items}' 的总价和货币\n\n示例：\n• `{items} 500 UAH`");
            ...
            m.insert("refund_approved", "✅ 已在 Squads v4 中创建退款提议！\n• 收据: #{invoice_id}");
            m.insert("invoice_already_cancelled", "⚠️ 收据 #{invoice_id} 已取消或已付款。");
            m.insert("unauthorized_approve", "⛔ 未授权：只有店铺经理可以批准 Squads v4 退款提议。");
            m.insert("squads_refund_approved", "✅ Squads v4 退款提议 #{proposal_index} 已批准！");
            m.insert("unauthorized_reject", "⛔ 未授权：只有店铺经理可以拒绝 Squads v4 退款提议。");
            m.insert("squads_refund_rejected", "🚫 Squads v4 退款提议 #{proposal_index} 已被拒绝。收据已恢复为 'paid'。");
            m.insert("refund_prompt", "♻️ 请输入要退款的收据 ID（例如：INV-101）：");
        ...
        // ── ar ──────────────────────────────────────────────────────────
        {
            ...
            m.insert("refund_initiated", "🔄 تم طلب الاسترداد!\nالفاتورة #{invoice_id}\nالفهرس: {proposal_idx}");
            m.insert("refund_error", "⚠️ خطأ في الاسترداد: {error_msg}");
            m.insert("unsupported_currency", "❌ خطأ: عملة غير مدعومة '{currency}'");
            ...
            m.insert("wallet_hint", "📱 امسح باستخدام Phantom أو Solflare أو أي محفظة Solana");
            m.insert("lang_confirm", "🌐 تم تغيير لغة الواجهة بنجاح إلى {flag} {lang_name}!");
            ...
            m.insert("price_needed", "✍️ يرجى تحديد السعر والعملة لـ '{items}'\n\nمثال:\n• `{items} 500 UAH`");
            ...
            m.insert("refund_approved", "✅ تم إنشاء اقتراح الاسترداد في Squads v4!\n• الإيصال: #{invoice_id}");
            m.insert("invoice_already_cancelled", "⚠️ الإيصال #{invoice_id} تم إلغاؤه بالفعل أو تم دفعه.");
            m.insert("unauthorized_approve", "⛔ غير مصرح: يمكن لمدير المتجر فقط الموافقة على مقترحات استرداد Squads v4.");
            m.insert("squads_refund_approved", "✅ تمت الموافقة على اقتراح استرداد Squads v4 #{proposal_index}!");
            m.insert("unauthorized_reject", "⛔ غير مصرح: يمكن لمدير المتجر فقط رفض مقترحات استرداد Squads v4.");
            m.insert("squads_refund_rejected", "🚫 تم رفض اقتراح استرداد Squads v4 #{proposal_index}. تمت استعادة الإيصال إلى 'paid'.");
            m.insert("refund_prompt", "♻️ يرجى إدخال معرف الإيصال لاسترداد المبلغ (مثال: INV-101):");
        ...
        // ── hi ──────────────────────────────────────────────────────────
        {
            ...
            m.insert("refund_initiated", "🔄 रिफंड का अनुरोध किया गया!\nबीजक #{invoice_id}\nइंडेक्स: {proposal_idx}");
            ...
            m.insert("wallet_hint", "📱 Phantom, Solflare या किसी भी Solana वॉलेट से स्कैन करें");
            m.insert("lang_confirm", "🌐 इंटरफ़ेस भाषा सफलतापूर्वक {flag} {lang_name} में बदल दी गई!");
            ...
            m.insert("price_needed", "✍️ कृपया '{items}' के लिए कुल मूल्य और मुद्रा निर्दिष्ट करें\n\nउदाहरण:\n• `{items} 500 UAH`");
            ...
            m.insert("refund_approved", "✅ Squads v4 में रिफंड प्रस्ताव बनाया गया!\n• रसीद: #{invoice_id}");
            m.insert("invoice_already_cancelled", "⚠️ रसीद #{invoice_id} पहले ही रद्द कर दी गई है या उसका भुगतान हो चुका है।");
            m.insert("unauthorized_approve", "⛔ अनधिकृत: केवल स्टोर प्रबंधक ही Squads v4 रिफंड प्रस्तावों को अनुमोदित कर सकता है।");
            m.insert("squads_refund_approved", "✅ Squads v4 रिफंड प्रस्ताव #{proposal_index} अनुमोदित किया गया!");
            m.insert("unauthorized_reject", "⛔ अनधिकृत: केवल स्टोर प्रबंधक ही Squads v4 रिफंड प्रस्तावों को अस्वीकार कर सकता है।");
            m.insert("squads_refund_rejected", "🚫 Squads v4 रिफंड प्रस्ताव #{proposal_index} अस्वीकार कर दिया गया। रसीद 'paid' पर बहाल कर दी गई।");
            m.insert("refund_prompt", "♻️ कृपया रिफंड के लिए रसीद ID दर्ज करें (जैसे, INV-101):");
