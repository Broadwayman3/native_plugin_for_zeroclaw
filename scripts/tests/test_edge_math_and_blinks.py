...
from pos_backend import handle_actions_spec_json, handle_action_get_invoice, handle_cancel_invoice, POSApiHandler, MAX_PAYLOAD_BYTES
...
def test_257_telegram_refund_checkpoint_inline_keyboard():
    ...
    payload = get_refund_checkpoint_inline_keyboard(refund_id=5)
...
def test_265_sqlite_parameterized_null_value_handling():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT OR REPLACE INTO invoices (id, reference_pubkey, fiat_currency, fiat_amount, usdc_amount, status, tx_signature) VALUES ('INV-NULL-1', 'RefNull1', 'USD', 10.0, 10.0, 'pending', ?)",
            (None,),
        )
        conn.commit()
...
def test_267_truncate_for_context_nested_structures():
    ...
    nested_payload = {"status": "confirmed", "usdc_amount": 10.0, "items": ["coffee", "tea"], "metadata": {"nested_key": "A" * 1000}}
...
def test_272_refund_request_reentrancy_lock_repeat():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT OR REPLACE INTO invoices (id, reference_pubkey, fiat_currency, fiat_amount, usdc_amount, status) VALUES ('INV-REF-2', 'RefR2', 'USD', 10.0, 10.0, 'paid')"
        )
        conn.commit()
...
def test_280_master_benchmark_pass_280_of_280():
    ...
    ixs = generate_atomic_refund_instructions(
        "Vault111111111111111111111111111111111111111", "Proposer11111111111111111111111111111111111", "Recipient1111111111111111111111111111111111", 10.0
    )
    assert len(ixs) >= 1
...
def test_295_durable_nonce_instruction_index_0_ordering():
    ...
    ixs_with_budget = [{"instruction": "SetComputeUnitPrice", "micro_lamports": 1000}, {"instruction": "SetComputeUnitLimit", "units": 200000}] + ixs
