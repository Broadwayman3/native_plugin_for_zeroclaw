...
def setup_test_db():
    ...
    cursor.executemany("INSERT OR REPLACE INTO nonce_accounts (pubkey, status) VALUES (?, 'free')", [
...
def test_048_durable_nonce_ttl_expiry_reclaim():
    ...
    try:
        ...
        cursor.execute("UPDATE nonce_accounts SET status = 'locked', locked_at = datetime('now', '-20 minutes') WHERE pubkey = 'Nonce111111111111111111111111111111111111111'")
        conn.commit()
        conn.close()
