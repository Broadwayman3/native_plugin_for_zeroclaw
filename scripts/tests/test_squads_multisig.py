...
def test_123_automatic_expired_pending_invoice_cleanup():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS invoices (id TEXT PRIMARY KEY, reference_pubkey TEXT, fiat_currency TEXT, fiat_amount REAL, usdc_amount REAL, status TEXT, created_at TIMESTAMP, updated_at TIMESTAMP);"
        )
        cursor.execute(
            "INSERT OR REPLACE INTO invoices (id, reference_pubkey, fiat_currency, fiat_amount, usdc_amount, status, created_at, updated_at) VALUES ('INV-EXP-99', 'RefExp99', 'USD', 10.0, 10.0, 'pending', datetime('now', '-25 hours'), datetime('now', '-25 hours'))"
        )
        conn.commit()
...
def test_131_recursive_inner_instructions_spl_parsing():
    mock_complex_tx = {
        "meta": {
            "err": None,
            "innerInstructions": [{"instructions": [{"parsed": {"type": "transfer", "info": {"destination": "MerchantUSDC_ATA", "amount": "10000000"}}}]}],
        },
        "transaction": {"message": {"instructions": [{"program": "compute-budget"}]}},
    }
...
def test_135_shell_scripts_executable_permission():
    ...
    script_paths = [
        repo_root / "scripts" / name for name in ["setup.sh", "build_wasm.sh", "verify_all.sh", "pre_commit.sh", "test_wasm_host.py", "lint_safety_ast.py"]
    ]
...
def test_141_post_pre_token_balance_delta_verification():
    ...
    mock_delta_tx = {
        "meta": {
            "err": None,
            "preTokenBalances": [{"accountIndex": 1, "mint": USDC_MINT, "uiTokenAmount": {"amount": "5000000"}}],
            "postTokenBalances": [{"accountIndex": 1, "mint": USDC_MINT, "uiTokenAmount": {"amount": "15000000"}}],
        },
        "transaction": {"message": {"accountKeys": ["Payer11111111111111111111111111111111111111", "MerchantUSDC_ATA"], "instructions": []}},
    }
