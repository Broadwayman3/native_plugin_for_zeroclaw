...
def test_163_x402_machine_commerce_402_header_parsing():
    ...
    x402_headers = {"X-PAYMENT-REQUIRED-AMOUNT": "1.00 USDC", "X-PAYMENT-RECIPIENT": "8xAZmQ1111111111111111111111111111111111111"}
    ...
    try:
        ...
        def worker(idx):
            try:
                ...
                c.execute(
                    "CREATE TABLE IF NOT EXISTS invoices (id TEXT PRIMARY KEY, reference_pubkey TEXT, fiat_currency TEXT, fiat_amount REAL, usdc_amount REAL, status TEXT);"
                )
                c.execute(
                    "INSERT OR REPLACE INTO invoices (id, reference_pubkey, fiat_currency, fiat_amount, usdc_amount, status) VALUES (?, ?, 'USD', 10.0, 10.0, 'pending')",
                    (f"INV-STRESS-{idx}", f"RefStress{idx:030d}"),
                )
                c.commit()
                c.close()
...
def test_190_wasm_binary_ram_cache_warmup_engine():
    from pos_core import load_wasm_binary_ram_cache, format_pubkey_short, get_solscan_tx_url, format_itemized_receipt, get_db_connection
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO invoices (id, reference_pubkey, fiat_currency, fiat_amount, usdc_amount, status) VALUES ('inv_cancel_test', 'Ref111', 'USD', 10.0, 10.0, 'pending')"
        )
        conn.commit()
    finally:
        conn.close()
