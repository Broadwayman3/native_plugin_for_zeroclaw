...
def test_237_refund_reentrancy_atomic_status_lock():
    ...
    try:
        conn = get_db_connection(TEST_DB_PATH)
        cursor = conn.cursor()
        cursor.execute("INSERT OR REPLACE INTO invoices (id, reference_pubkey, fiat_currency, fiat_amount, usdc_amount, status, created_at, updated_at) VALUES ('INV-R1', 'RefR1', 'USD', 10.0, 10.0, 'paid', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);")
        conn.commit()
