...
def handle_actions_spec_json(handler, query_params):
    return 200, {"rules": [{"pathPattern": "/api/v1/actions/**", "apiPath": "/api/v1/actions/**"}]}
...
def handle_action_get_invoice(handler, query_params):
    ...
    action_payload = {
        ...
        "links": {"actions": [{"label": "Pay Now", "href": f"/api/v1/actions/pay_invoice?invoice_id={raw_id}"}]},
    }
    extra_headers = {"X-Action-Version": "2.1.3", "X-Blockchain-Ids": "solana:EtWTRABZaYqXxicM2Tz2fSpo5nszvh6wT9D3gYqH1cQ"}
...
class POSApiHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.headers.get("X-ACCEPT-PAYMENT") == "x402" and "/api/v1/sales/premium_analytics" in self.path:
            extra_headers = {"X-PAYMENT-REQUIRED-AMOUNT": "1.00 USDC", "X-PAYMENT-RECIPIENT": "8xAZmQ1111111111111111111111111111111111111"}
            body = {
...
def run_server(port=8080, host=None, seed_sample_data=True):
    ...
    if not host or not str(host).strip():
        host = os.getenv("HOST") or os.getenv("POS_HOST") or "0.0.0.0"  # nosec B104 -- intentional: POS HTTP server must accept LAN POS-terminal clients
    host = host.strip()
    ...
    try:
        ...
        if getattr(e, "errno", None) == 98 or "already in use" in str(e).lower():
            ...
            try:
                ...
                print(
                    f"❌ [POS Server Error] Both PORT={port - 1} and PORT={fallback_port} are already in use. Please free a port or set PORT environment variable. Exiting safely."
                )
                sys.exit(1)
        else:
            raise
