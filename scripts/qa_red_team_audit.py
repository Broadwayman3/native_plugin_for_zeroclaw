...
def test_criterion_4_cors_preflight():
    ...
    sock = DummySocket()
    handler = POSApiHandler(sock, ('127.0.0.1', 12345), DummyServer())
...
def test_criterion_5_invoice_id_filtering_and_port():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("INSERT INTO invoices (id, reference_pubkey, fiat_currency, fiat_amount, usdc_amount, status) VALUES ('INV-QA-101', 'Ref1', 'USD', 10.0, 10.0, 'paid')")
        cursor.execute("INSERT INTO invoices (id, reference_pubkey, fiat_currency, fiat_amount, usdc_amount, status) VALUES ('INV-QA-102', 'Ref2', 'USD', 20.0, 20.0, 'pending')")
        conn.commit()
    finally:
        conn.close()
