...
def cleanup_expired_pending_invoices(conn: Optional[sqlite3.Connection] = None, db_path: str = DB_PATH) -> None:
    ...
    with get_db_cursor(conn, db_path) as cursor:
        cursor.execute(
            "UPDATE invoices SET status = 'expired', updated_at = CURRENT_TIMESTAMP WHERE status = 'pending' AND created_at < datetime('now', '-24 hours')"
        )
    ...
    with get_db_cursor(conn, db_path) as cursor:
        ...
        if cursor.fetchone()[0] == 0:
            ...
            sample_data = [
                ...
                (
                    "INV-102",
                    "8xRefKey22222222222222222222222222222222222",
                    "UAH",
                    150.0,
                    3.61,
                    "paid",
            ...
            cursor.executemany(
                "INSERT INTO invoices (id, reference_pubkey, fiat_currency, fiat_amount, usdc_amount, status, tx_signature, customer_address, pix_id, pix_payload, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                sample_data,
            )
...
def init_db(db_path: str = DB_PATH, seed_sample_data: bool = True) -> None:
    ...
    try:
        ...
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS sop_checkpoints (id TEXT PRIMARY KEY, sop_id TEXT NOT NULL, step_id TEXT NOT NULL, state_data TEXT, status TEXT NOT NULL DEFAULT 'pending', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)"
        )

        cursor.execute("SELECT COUNT(*) FROM nonce_accounts")
        if cursor.fetchone()[0] == 0:
            cursor.executemany(
                "INSERT INTO nonce_accounts (pubkey, status) VALUES (?, 'free')",
                [
...
def get_sales_summary_stats(db_path: str = DB_PATH) -> Dict[str, Any]:
    ...
    try:
        ...
        cursor.execute(
            "SELECT fiat_currency, COUNT(*) as count, SUM(fiat_amount) as total_fiat, SUM(usdc_amount) as total_usdc FROM invoices WHERE status = 'paid' GROUP BY fiat_currency"
        )
        by_curr = {
            r["fiat_currency"]: {"count": r["count"], "total_fiat": round(r["total_fiat"] or 0.0, 2), "total_usdc": round(r["total_usdc"] or 0.0, 2)}
...
def create_invoice_record(data: Dict[str, Any], db_path: str = DB_PATH) -> Tuple[bool, str]:
    ...
    try:
        ...
        """,
            (
                data["id"],
                data["reference_pubkey"],
                data.get("fiat_currency", "USD"),
                data.get("fiat_amount", data["usdc_amount"]),
                data["usdc_amount"],
                now,
                now,
            ),
...
def create_squads_proposal(conn: sqlite3.Connection, invoice_id: str, recipient_pubkey: str, amount_usdc: float) -> int:
    """Creates a Squads v4 multisig refund proposal record. Returns auto-increment proposal_index."""
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO squads_proposals (invoice_id, recipient_pubkey, amount_usdc, status) VALUES (?, ?, ?, 'created')",
        (invoice_id, recipient_pubkey, amount_usdc),
    )
    conn.commit()
    return cursor.lastrowid or 0


def update_squads_proposal_status(conn: sqlite3.Connection, proposal_index: int, status: str) -> bool:
    """Updates Squads v4 proposal status (approved/rejected). Returns True if row found."""
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE squads_proposals SET status = ? WHERE proposal_index = ?",
        (status, proposal_index),
    )
    conn.commit()
